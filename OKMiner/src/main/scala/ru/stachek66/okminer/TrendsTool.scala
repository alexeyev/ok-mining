...
object TrendsTool {
  ...
  def extractTrends(text: String): Iterable[String] = {
    ...
    //    val triples = allDuples.zip(if (tokens.size > 2) tokens.tail.tail else Seq()).filter {
    ...
    //    }
    //
    //    val rankedTriples = tResults.toSeq.sortBy(-_._2).take(5).mkString("\n")
    //    log.debug("Triples:\n" + rankedTriples)
  ...
  //  println(extractTrends(testText))
}