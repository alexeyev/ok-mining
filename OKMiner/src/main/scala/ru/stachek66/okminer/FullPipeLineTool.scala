...
object FullPipeLineTool extends App {
  ...
  val tokens = filtered.map(Tokenizer.tokenize(_)).flatten //filtered.map(Tokenizer.tokenize(_))
  ...
  log.info(
    dResults.toSeq.sorted {
      case (t0, t1) => t0._3 > t1._3
    }.toString)
}
